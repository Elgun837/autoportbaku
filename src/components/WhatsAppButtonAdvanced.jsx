...
const WhatsAppButtonAdvanced = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const hasScrolledEnough = window.scrollY > 300;
      const footerVisible = checkFooterVisible();

      if (hasScrolledEnough && !footerVisible) {
        setIsVisible(true);
      } else {
        setIsVisible(false);
      }
  ...
  try {
    ...
    console.warn(
      "WhatsApp Button Advanced: Language context not available, using fallback"
    );
    t = null;
    lang = "en";
  }
